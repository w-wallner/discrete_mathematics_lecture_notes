...
The edges of a graph correspond to a \emph{relation} on $V$. In case of undirected graphs, this relation is symmetric.
Graphs can have \textbf{multiple edges}, in which case $E$ is a multiset.
An overview of these concepts is given in \cref{fig:GraphTypes}.
...
\begin{bclogo}[logo=\bcinfo,barre=none,noborder=false]{Remark}
~\\Unless otherwise stated we assume that all graphs are \textbf{simple} and \textbf{finite} throughout this lecture.
\end{bclogo}


Graph theory notation: \\[\medskipamount]
\begin{tabular}{ll}
  $V$, $V(G)$ & vertice set of G \\
  $E$, $E(G)$ & edge set of G \\
  $\alpha_0 = |V|$ & number of vertices \\
  $\alpha_1 = |E|$ & number of edges \\
  $\alpha_2 =    $ & number of faces (will be defined later) \\ % TODO: Add ref
\end{tabular}

\begin{figure}[t!htp]
\centering
\begin{tabular}{c c}
\subfloat[directed graph] {
    \includegraphics[width=.35\textwidth]{01_graph_theory/pics/directed-graph_edge.pdf}
} &
\subfloat[undirected graph] {
    \includegraphics[width=.35\textwidth]{01_graph_theory/pics/graph_edge.pdf}
} \\
\subfloat[node with a loop] {
\makebox[.45\textwidth]{
    \includegraphics[width=.10\textwidth]{01_graph_theory/pics/graph_loop.pdf}
    }
} &
\subfloat[multiple edges] {
    \includegraphics[width=.35\textwidth]{01_graph_theory/pics/graph_multiple-edges.pdf}
}
\end{tabular}
\caption{Visualization of different graph types}
...
\begin{figure}[htb]
\centering
\subfloat[vertex $v$ with degree $d(v)=6$]{
	\includegraphics[scale=.5]{01_graph_theory/pics/graph_degree.pdf}
}
\subfloat[vertex $v$ with in-degree $d^{-}(v)=3$ and out-degree $d^{+}(v)=3$]{
...
\begin{figure}[htb]
\centering
\subfloat[undirected graph, each edge is counted twice]{
	\includegraphics[scale=.5]{01_graph_theory/pics/graph_degree_handshaking-lemma.pdf}
}
\subfloat[directed graph, each edge is counted only once]{
