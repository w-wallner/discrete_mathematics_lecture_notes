...
Finite Set $\{a_1, \ldots, a_n\} = A$, $n$-element set or $n$-set
...
\begin{enumerate} %TODO start with 5
...
\[
  ...
  \forall n \in \bathbb{N}, \forall x,y in \mathbb{C}
\]
...
\Proof.
Take a permutation $\pi = (1 \ldots )(ldots) \ldots (\ldots) \in S_n, k \text{ cycles}$.

How many such permutations are there?

If $1$ is a fixed point: $s_{n-1, k-1}$.

If $1$ is not a fixed point, start with a permutation of $n-1$ elements and add the element $1$ to one of the cycles. We can insert it before any of the $n-1$ elements, thus we get
