...
\textbf{Example}.
Sets $A_1, A_2, \ldots A_m \subseteq M$. We take the powerset of set of indices "(2^{\{1,2, \ldots , m\}}, \superseteq)$. Note that the Möbius Function only depends on the structure of the order. 

\[
    \mu (A,B) = \begin{cases} (-1)^{|B| - |A| &, A\superseteq B\\ 0 &, otherwise \end{cases}
\]

\[
    I \subseteq \{ 1,2, \ldots, m}, f(I) = | \bigcap_{i\in I} A_i \cap \bigcap_{j \in \[1,\ldots,m\} \backslash I} \overline{A}_j |
\]
\begin{gather*}
    S_f(I) = \sum_{J \superseteq I} f(J) = \left| \bigcap_{i\in I} A_i  \right| \\
    \left[ S_f(x) = \sum_{z\leq x} f(z) \right]
\end{gather*}

\begin{gather*}
    f(I) &= \sum_{J \superseteq I} S_f(I) \mu(J,I} \\
        &= \sum_{J \superseteq I} (-1)^{|J| - |I|} * \left| \bigcap_{i\in I} A_i \right|
...
\Remark.
\[
    a \leq b \Rightleftarrow a \vee b = b \Rightleftarrow a \wedge b = a
\]
...
\Proof.
$\s \wedge t$ greatest common lower bound of $s,t$. $ x,y are lower bounds of $s,t" $\Rightarrow$ $x \leq s \wedge t, y \leq s \wedge t$. 
...
\begin{align*
    ...
    \alpha, \beta \in \Pi_n: & \alpha \wedge \beta: i,j \in \{1,2,\ldots,n\} \text{in the same block $\implies$ they are in the same block of both $\alpha$ and $\beta$ \\
...
\Proof.
$(*) \equals \sum_{x: x\wedge b = 0} \mu(x,1) = 0$
, since $0 \wedge b = 0$
...
\begin{align*}
    ...
    \Rightarrow^{\text{MIF}} N(b) = \sum_{y \leq b} S(y) \mu(y,b) = 0\\
    N(0) = 0, \text{ but } N(0) = \sum_{x:x \wedge b = 0} \mu(x,1) = 0 \\
\end{align*}
\TODO{replace $\Rightarrow^{\text{MIF}}$ with the text over the $\Rightarrow$}
...
\begin{definition}
$a,b \in \mathbb{Z}: a\mid b: \Leftrightarrow \exits c \in \mathbb{Z}: a * c = b$
